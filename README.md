...
- 使用 `/2fa#密鑰`：瀏覽器不會將 `#` 後面的片段隨頁面請求傳送。完整連結仍可能留在瀏覽器歷史，請勿公開分享。
...
網路上有很多類似的在線工具，還能以直鏈取得代碼。但如果你不知道的話，只要訪問直接帶密鑰的網址，例如 `?secret=密鑰`，瀏覽器會把密鑰隨 HTTP 請求傳送出去，對方的遠端網站服務或 CDN 能收到這份資料，也能透過存取日誌、監控或應用程式記錄並留存。也就是說，這些請求中的密鑰可能被明文儲存。當然，是否實際留存取決於對方的實作、設定與隱私政策:))

這是我公開的第一個 Vibe coding 輪子項目，有感而發寫了些文字，感興趣可以到[主頁](https://github.com/LZSMIAO)看看。
